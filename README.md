...
Notes: if the GPU is not as powerful as A100, the generation time needed may exceed 1 minutes. In this case we need to bump the timeout setting higher by applying custom configuration using `BENTOML_CONFIG=bentoml_configuration.yaml bentoml serve service:svc`.
